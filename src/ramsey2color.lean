...
begin
suffices additive_version : ∀ NM N M s t : ℕ, (N + M) = NM → Ramsey_prop N s.succ t.succ.succ → Ramsey_prop M s.succ.succ t.succ → Ramsey_prop (N + M) s.succ.succ t.succ.succ,
intros,
apply (additive_version (N + M) N M s t),
trivial,
assumption,
assumption,
intro,
induction NM with NM RamseynM,
intros _ _ _ _ NplusM RamseyN,
simp at NplusM,
rw [NplusM.left] at RamseyN,
unfold Ramsey_prop at RamseyN,
cases RamseyN.left,
intros _ _ _ _ NplusM RamseyN RamseyM,
unfold Ramsey_prop,
split,
simp [NplusM],
intro,
haveI mrcl : has_zero (fin (N + M)),
rw NplusM,
apply fin.has_zero,
...
have temp:=  lt_of_le_of_lt qflrpos smallqat,
simp at temp,
exact temp,
--have qflreq0 :⌊q⌋ = 0,
--rw int.floor_eq_iff,
...
intro zeroinS',
simp [S'] at zeroinS',
rcases zeroinS' with ⟨a, aprop, ftransa⟩,
have ftransat := (ftrans ⟨a, finset.mem_univ a⟩).prop,
simp [ftransa] at ftransat,
exact ftransat,
rw finset.card_insert_of_not_mem znotinS',
simp [S'],
rw Sclique.card_eq,
